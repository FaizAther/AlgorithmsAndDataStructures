...
public class QuaternaryHeapsortTest {
    ...
    public void testSortGeneric(int size) {
        Integer[] arr = new Integer[size];
        Random rd = new Random();
        for (int i = 0; i < size; i++) {
            arr[i] = rd.nextInt();
        }
        Integer[] arr_sorted = arr.clone();
        Arrays.sort(arr_sorted);
        QuaternaryHeapsort.quaternaryHeapsort(arr);
        assertArrayEquals(arr, arr_sorted);
    }

    @Test
    public void testSort1() {
        int[] sizes = {0,1,5,25,55, 100, 500, 1000, 5000, 7500, 9500, 9999, 10000};
        for (int c : sizes) {
            testSortGeneric(c);
        }
    }
